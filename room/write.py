...
- ハッシュタグの直前に、「楽天市場で見る」ボタンを押すよう軽く一言添える。
  「気になったらチェックしてみてね」のような温度感でよい。説明的にならないこと。

■ ハッシュタグ
フィード本文としては見えませんが、ROOMのハッシュタグ検索・タグ一覧からの
流入経路になります。指定された個数まで、次の観点をバランスよく含めてください。
- 商品そのもの（例: #デスクマット #掃除グッズ）
- 使う場面・目的（例: #時短家事 #おうち時間）
- 想定される読み手（例: #一人暮らし #新生活）
自分で購入・使用したことを示すタグ（例: #買ってよかった #オリジナル写真）は、
実際に自分で使った体験ではない以上、付けない。「実際に自分が使った体験を
創作しない」という上のルールはハッシュタグにも適用される。
